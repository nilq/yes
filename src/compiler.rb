...
class Compiler
  ...
  def initialize(mod=nil, function=nil)
    @module = mod || LLVM::Module.new("yes")
    @locals = {}
  ...
  def function(name)
    func = @module.function.add(name, [], LLVM.Void)
    compiler = Compiler.new(@module, func)
    yield compiler
    compiler.finish
  end
...
class CallNode
  def llvm_compile(compiler)
    raise "Receiver fucking up compilation" if receiver

    compiled_arguments = arguments.map { |arg|
      arg.llvm_compile(compiler)
    }
...
class FuncNode
  def llvm_compile(compiler)
    raise "Parameters fucking up compilation" if !params.empty?
